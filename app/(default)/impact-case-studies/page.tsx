...
interface ImpactCaseStudiesPageProps extends PageProps<"/impact-case-studies"> {}

export async function generateMetadata(_props: ImpactCaseStudiesPageProps): Promise<Metadata> {
...
export default async function ImpactCaseStudiesPage(
	...
	const response = await client.impactCaseStudies.list({ limit, offset });
		...
		<Main className="container flex flex-1 flex-col gap-8 px-8 py-12 xs:px-16">
			...
			>
				{items.map((item) => {
					...
					const href = `/impact-case-studies/${slug}`;
