...
export async function generateMetadata(): Promise<Metadata> {
	const t = await getTranslations("PartnershipsAndCollaborations");
...
export default async function PartnershipsAndCollaborationsPage(): Promise<ReactNode> {
	const response = await client.pages.bySlug({ slug: "partnerships-and-collaborations" });
	const breadcrumbs = navigation().breadcrumbs.partnershipsAndCollaborations;
	const {
		data: { title, content },
	} = response;
