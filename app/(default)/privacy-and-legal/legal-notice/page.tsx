...
export default async function LegalNoticePage(): Promise<ReactNode> {
	const response = await client.pages.bySlug({ slug: "legal-notice" });
	const breadcrumbs = navigation().breadcrumbs.legalNotice;
	const {
		data: { title, content },
	} = response;
