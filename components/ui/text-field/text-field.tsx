...
export function TextField(props: Readonly<TextFieldProps>): ReactNode {
	...
	return (
		<AriaTextField className="group flex flex-col gap-1" {...rest}>
			<Label className="text-regular text-black">{label}</Label>
			<div
				className={cn(
					"flex items-center gap-2.5 px-3.25 pt-4.5 pb-4 text-regular text-black placeholder-gray-700 text-[16px]",
					"border-b-2 border-gray-200 border-b-gray-400 bg-gray-200",
					"hover:border-b-primary hover:bg-gray-200",
					"focus-within:pt-4 focus-within:px-2.75 focus-within:border-2 focus-within:border-primary hover:focus-within:border-primary",
				)}
			>
				{startIcon}
